...
def main():
    ...
    # Prepare X by first dropping the target column 'IP' (if it exists)
    # from the DataFrame that will be used for features,
    # and then selecting only the required_features.
    X_data = poultry_data.drop(['IP'], axis=1, errors='ignore') 
    X = X_data[required_features].copy()
    ...
    model_filename = 'poultry_rf_model.joblib'
